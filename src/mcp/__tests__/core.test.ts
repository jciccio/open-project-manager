...
describe("MCP Server Core Tools", () => {
  ...
  it("scopes get_card_by_identifier to the requesting user's own project", async () => {
    ...
    try {
      ...
      const mineCard = await executeMcpTool(
        "create_card",
        { projectId: mineProj.project!.id, columnId: mineColId, title: "My Card" },
        { userId }
      );
      await executeMcpTool(
        "create_card",
        { projectId: theirsProj.project!.id, columnId: theirsColId, title: "Their Card" },
        { userId: otherUser.id }
      );

      const lookup = await executeMcpTool("get_card_by_identifier", { identifier: "SAME-1" }, { userId });
